...
program    : PROGRAM IDENTIFIER LPAREN idlist RPAREN SEMICOLON vblock DOT { parseresult = makeprogram($2, $4, $7); } ;
  ...
  statement  :  BEGINBEGIN statement endpart   { $$ = makeprogn($1,cons($2, $3)); }
             ...
             |  assignment
             |  FOR assignment TO expr DO statement   { $$ = makeforl($1, $2, $4, $6); }
             ;
  funcall    :  IDENTIFIER LPAREN expr_list RPAREN    { $$ = makefuncall(); }
             ;
...
TOKEN makeforl(TOKEN tok, TOKEN assign, TOKEN expr, TOKEN statements) {
    ...
    TOKEN leoper = makeoperator(LEOP);
    ifs = makeif(ifs, leoper, body, NULL);
    TOKEN iden = makecopy(assign->operands);
    TOKEN iden2 = makecopy(iden);
    TOKEN iden3 = makecopy(iden);
    iden->link = expr;
    leoper->operands = iden;

    TOKEN assgn = makeoperator(ASSIGNOP);
    TOKEN increment = makeoperator(PLUSOP);
